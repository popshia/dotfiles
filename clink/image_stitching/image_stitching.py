...
class Stitcher:
    ...
    def matchKeyPoint(self, kps_l, kps_r, features_l, features_r, ratio):
        ...
        Match_idxAndDist = (
            []
        )  # min corresponding index, min distance, seccond min corresponding index, second min distance
        for i in range(len(features_l)):
            min_IdxDis = [
                -1,
                np.inf,
            ]  # record the min corresponding index, min distance
            secMin_IdxDis = [
                -1,
                np.inf,
            ]  # record the second corresponding min index, min distance
        ...
        for idx, correspondingIdx in goodMatches:
            ...
            psB = (
                int(kps_r[correspondingIdx].pt[0]),
    ...
    def fitHomoMat(self, matches_pos):
        ...
        for run in range(NumIter):
            SubSampleIdx = random.sample(
                range(NumSample), NumRamdomSubSample
            )  # get the Index of ramdom sampling
            H = homography.solve_homography(
                srcPoints[SubSampleIdx], dstPoints[SubSampleIdx]
            )
...
if __name__ == "__main__":
    # fileNameList = [("hill1", "hill2"), ("1", "2"), ("img1", "img2"), ("S1", "S2")]
    fileNameList = [("hill1", "hill2")]

    for sets in fileNameList:
        # Read the img file
        src_path = "img/"
        fileName1 = sets[0]
        fileName2 = sets[1]
        ...
        plt.imshow(img_left[:, :, ::-1].astype(int))
        plt.show()
        plt.imshow(img_right[:, :, ::-1].astype(int))
        plt.show()
